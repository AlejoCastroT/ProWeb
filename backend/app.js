...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign(
            { userId: user._id, username: user.username },  // Payload con el id y nombre de usuario
            process.env.JWT_SECRET || 'secret_key',  // Clave secreta (puedes configurarla en tu archivo .env)
        ...
        res.status(200).json({
            ...
            token: token,  // Incluimos el token
        });
    } catch (error) {
